...
`);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let target of targets) {
                if (target === "zip" || target === "7z" || target.startsWith("tar.")) {
                    ...
                    promises.push(this.archiveApp(target, appOutDir, destination).then(() => this.dispatchArtifactCreated(destination, this.generateName(target, arch, true))));
                }
            }
    ...
    buildPackage(destination, target, options, appOutDir, arch) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const projectUrl = yield this.appInfo.computePackageUrl();
            if (projectUrl == null) {
            ...
            const args = ["-s", "dir", "-t", target, "--architecture", arch === metadata_1.Arch.ia32 ? "i386" : "amd64", "--name", this.appInfo.name, "--force", "--after-install", scripts[0], "--after-remove", scripts[1], "--description", platformPackager_1.smarten(target === "rpm" ? this.buildOptions.description : `${ synopsis || "" }\n ${ this.buildOptions.description }`), "--maintainer", author, "--vendor", options.vendor || author, "--version", this.appInfo.version, "--package", destination, "--url", projectUrl];
            if (target === "deb") {
            ...
            util_1.use(this.appInfo.buildNumber, it => args.push("--iteration", it));
